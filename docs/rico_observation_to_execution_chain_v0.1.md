...
a runtime integrity assessment;
an admissibility posture;
an execution boundary;
a documented governance rationale;
a lightweight governance receipt;
sufficient material to reconstruct the relationship between observation, authority, consequence and outcome.
Expected Boundary
NeoMundi	RiCo
...
required receipt attributes;
the smallest executable mapping from observation to admissibility assessment.
